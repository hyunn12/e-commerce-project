...
@SpringBootTest
class UserCouponUseServiceConcurrencyTest {
    // orm--
    @Autowired
    ...
    // sut --
    @Autowired
    private UserCouponUseService userCouponUseService;

    @DisplayName("낙관적 락 적용 시, 동시에 쿠폰을 사용해도 한 번만 사용된다.")
    @Test
    void getDetailWithLock() throws InterruptedException {
        // arrange
        ...
        for (int i = 0; i < threadCount; i++) {
            executor.submit(() -> {
                try {
                    userCouponUseService.use(userCoupon.getId(), userCoupon.getUserId(), 10000);
